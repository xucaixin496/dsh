 ...
 * always survives; the optional server-side extracted text follows it when
 * present. Text-only adapters call this so a file attachment is never
 * silently erased from the wire conversation.
...
export function fileBlockText(block: ContentBlock): string {
  ...
  const header = `[Attachment: ${name} (${block.attachment.mediaType}, ${block.attachment.bytes} bytes)]`
